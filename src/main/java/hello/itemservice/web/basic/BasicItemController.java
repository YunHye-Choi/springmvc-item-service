...
//    @PostMapping("/add")
    public String addItemV2(@ModelAttribute("item"/*model에 지정되는 값 설정*/) Item item, Model model) {
        itemRepository.save(item);
//        model.addAttribute("item", item); // 자동으로 추가되어 생략 가능
        return "basic/item";
    }
...
//    @PostMapping("/add")
    ...
    public void init(){
        ...
        itemRepository.save(new Item("itemB", 15000, 6));
    }
}
